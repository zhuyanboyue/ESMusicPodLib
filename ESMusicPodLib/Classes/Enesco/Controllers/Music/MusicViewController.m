...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        _sharedMusicVC = [[UIStoryboard storyboardWithName:@"Music" bundle:[NSBundle bundleForClass:[self class]]] instantiateViewControllerWithIdentifier:@"music"];
...
- (void)createStreamer {
    ...
    if (_musicEntity.musicUrl.length) {
        track.audioFileURL = [NSURL URLWithString:_musicEntity.musicUrl];
    }else if (_musicEntity.fileName.length) {
        NSString *soundFilePath = [[NSBundle bundleForClass:[self class]] pathForResource:_musicEntity.fileName ofType: @"mp3"];
